...
if [[ $code =~ '"total":0' ]] ; then
	...
	curl -X PUT "elasticsearch:9200/_template/softov_log" -H 'Content-Type: application/json' -d @index_template.cfg
...
curl -X PUT "elasticsearch:9200/logstash*/_mapping?pretty" -H 'Content-Type: application/json' -d '
{
  "properties": {
        "message": { "type": "text"  },
		    "duration": { "type": "long"  },
...
curl -X PUT 'elasticsearch:9200/_ilm/policy/logstash_clean_policy?pretty' -H 'Content-Type: application/json' -d '
{ 
  "policy": {
    "phases": {
      "hot": {
...
curl -X PUT 'elasticsearch:9200/_template/logstash_clean_policy1?pretty' -H 'Content-Type: application/json' -d '
{
  "index_patterns": [
    "logstash*"
  ],
